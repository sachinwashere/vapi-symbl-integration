  ...
  vapiClient.on('connect', (vapiConnection) => {
    ...
    vapiConnection.on('message', async (data) => {
      if (data.type === 'binary') {
        ...
        const { leftChannel, rightChannel } = splitChannels(binaryData);

        if (customerConnection) {
          customerConnection.send(Buffer.from(leftChannel));
        }
        if (agentConnection) {
          agentConnection.send(Buffer.from(rightChannel));
        }
...
app.post('/start-call', async (req, res) => {
  ...
  try {
    ...
    const customerConnection = await connectToRTAEndpoint(sessionId, process.env.RTA_ID);
    const agentConnection = await connectToRTAEndpoint(sessionId, process.env.RTA_ID);
